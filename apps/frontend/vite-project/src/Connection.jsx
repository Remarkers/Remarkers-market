...
export function Connectiond() {
  ...
  const prevPage = () => {
    ...
    const providers = {Polkadot: "wss://rpc.polkadot.io", OnFinality: "wss://polkadot.api.onfinality.io/ws?apikey=023fc078-a5b7-4a72-81c4-40e118b6097b", Dwellir: "wss://polkadot-rpc.dwellir.com", Pinknode: "wss://public-rpc.pinknode.io/polkadot", RadiumBlock: "wss://polkadot.public.curie.radiumblock.co/ws", OneRPC: "wss://1rpc.io/dot", LuckyFriday: "wss://rpc-polkadot.luckyfriday.io", GeoDNS1: "wss://rpc.ibp.network/polkadot", GeoDNS2: "wss://rpc.dotters.network/polkadot", stakewood:"dot-rpc.stakeworld.io" }
    const [endpoint, setEndPoint] = useState(providers.Polkadot)
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="m3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75Z" />
      ...
      <Select label="Endpoints">
        ...
        <Option onClick={() => (selectProvider(providers.Pinknode))}>Hosted by Pinknode</Option>
        <Option onClick={() => (selectProvider(providers.RadiumBlock))}>Hosted by RadiumBlock</Option>
        <Option onClick={() => (selectProvider(providers.OneRPC))}>Hosted by 1RPC</Option>
        <Option onClick={() => (selectProvider(providers.LuckyFriday))}>Hosted by LuckyFriday</Option>
        <Option onClick={() => (selectProvider(providers.GeoDNS1))}>Hosted by IBP-GeoDNS1</Option>
        <Option onClick={() => (selectProvider(providers.GeoDNS2))}>Hosted by IBP-GeoDNS2</Option>
        <Option onClick={() => (selectProvider(providers.stakewood))}>Hosted by stakewood</Option>
      </Select>
    </div>
        )
        : <Menu>
        ...
        <MenuList>
          ...
          <MenuItem onClick={() => (selectProvider(providers.Pinknode))}>Hosted by  Pinknode</MenuItem>
          <MenuItem onClick={() => (selectProvider(providers.RadiumBlock))}>Hosted by  RadiumBlock</MenuItem>
          <MenuItem onClick={() => (selectProvider(providers.OneRPC))}>Hosted by 1RPC</MenuItem>
          <MenuItem onClick={() => (selectProvider(providers.LuckyFriday))}>Hosted by LuckyFriday</MenuItem>
          <MenuItem onClick={() => (selectProvider(providers.GeoDNS1))}>Hosted by IBP-GeoDNS1</MenuItem>
          <MenuItem onClick={() => (selectProvider(providers.GeoDNS2))}>Hosted by IBP-GeoDNS2</MenuItem>
          <MenuItem onClick={() => (selectProvider(providers.stakewood))}>Hosted by stakewood</MenuItem>
        </MenuList>
      </Menu>
      }
      {
            walletConnected? (
              ...
              icon={
                <span className="mx-auto mt-1.5 block h-3 w-3 rounded-full bg-green-900 content-['']" />
              }
            />
            )
            : <Chip
              ...
              <><div className="mb-6">
                <ul className="mt-3  -ml-2 flex flex-col gap-1">
                  ...
                  <ul className="mt-4 -ml-2.5 flex flex-col gap-1">
                    ...
                    <img
                      src="https://nftstorage.link/ipfs/bafybeid74tnsnmd2kq44xqx43sphszo2b5ihevdfm4ichgvklomsh4rc4u"
                      alt="Nova"
                      className="h-12 w-12" />
                    <Typography
                      className="uppercase"
...
Disconnect </button>
    ...
    <div key={account.address}>
      <MenuItem className="mb-4 flex items-center justify-center gap-3 !py-4 shadow-md" onClick={() => selectAccount(account)}>
      <Identicon
...
const [open, setOpen] = React.useState(false);
          ...
          const selectedAddress = async(account) => {
            ...
            const [api, setApi] = useState();
            const [accounts, setAccount] = useState()
            const [selectedAccount, setselectedAccounts] = useState(JSON.parse(localStorage.getItem("selectedAccount")))
console.log(JSON.parse(localStorage.getItem("selectedAccount")))
            const [isMobile, setIsMobile] = useState(false);
